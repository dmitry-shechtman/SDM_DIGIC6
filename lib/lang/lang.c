...
// NOTE: Filename below must be all capitals even though actual filenames lower case.

 if((strcmp(filename,"A/SDM/LANG/POLISH.LNG")==0)||(strcmp(filename,"A/SDM/LANG/HUN.LNG")==0)||(strcmp(filename,"A/SDM/LANG/ROMANIAN.LNG")==0))
{
  conf.font_cp = FONT_CP_WIN_1250;
}
 else
  conf.font_cp = FONT_CP_WIN_1252;
 font_init();

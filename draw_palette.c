...
if(current_orientation==1)sw-=120;                  // width is really screen height
    switch (palette_mode) 
     {
...
// large rectangle to display chosen colour
...
if (current_orientation == 2)// inverted
{
 xx=screen_width-x;
 yy=screen_height-y;
}
else if (current_orientation == 1 && current_rotation == 0)// rotated left
{
xx=screen_width-y-1;
yy=x;
}
else if (current_orientation == 1 && current_rotation == 1)// rotated right
{
 xx=y;
...
else
{
 ...
 offset=yy * screen_buffer_width + xx;   // no x-aspect correction, see draw_pixel_grid() below
#ifndef THUMB_FW
...
#error DRAW_ON_ACTIVE_BITMAP_BUFFER_ONLY is required for DIGIC 6 ports
#endif
    register int cli = cl ^ 0xffffffff;   // cli = 0xffffffff if cl = 0 (black)
    extern volatile char *opacity_buffer[];
    static unsigned int prev_offs = 0xffffffff;
    register unsigned int offs2;
    offset*=2;                            // double-height for DIGIC 6 tiny text
  for(i=0;i<2;i++)                       // each row
  {
   offset+=i*screen_buffer_width;
   for(j=0;j<2;j++)                     // adjacent pixels
   {
    offset+=j;
    offs2 = (offset>>1)<<2;
    if (cli != 0xffffffff)                          // not black
    {
        if (prev_offs != offs2)                    // new pair of U,V values
        {
 ...
 void draw_pixel_grid(coord x,coord y,color cl)  // no transformations applied
{
  unsigned int offsetx;
 int i,j; 
   int  offset=y * screen_buffer_width + x;   
#ifndef THUMB_FW
    // drawing on 8bpp paletted overlay
#ifdef DRAW_ON_ACTIVE_BITMAP_BUFFER_ONLY
	bitmap_buffer[active_bitmap_buffer][offset] = cl;
...
{
 if((draw_pixel_proc!=draw_pixel_grid)&&(current_orientation == 1))
 {
 ...
 draw_pixel_proc(2*x,y,cl);
 draw_pixel_proc(2*x+1,y, cl);  // Draw second pixel if screen scaling is needed
 #else
...
#endif
    draw_set_draw_proc(NULL);                            // draw_pixel_std
}
...
{
 char *start;                               // beginning of string
 start=(char*)s;
    ...
    return (s-start)*FONT_WIDTH;              // return string length in pixels  
}
...
int draw_char(coord x, coord y, const char ch, color cl, int m) 
{
  ...
  if(m>16)m=16;
    ...
    for (i=0; i<FONT_HEIGHT; i++)                                                         // source font row                                  
    {
	for (ii=0; ii<FONT_WIDTH; ii++)                                                     // source font column                             
      {
        for(j=0;j<m;j++)                                                                  // repeat pixel vertically
         {
           for(jj=0;jj<m;jj++)                                                            // repeat pixel horizontally
           {
...
void draw_big_string_centre_len(int x, int y, int len, const char *str, color cl) 
{
    ...
    const char *s=str;
     ...
     j = (len-((strlen(s)*BIG_FONT_WIDTH)))>>1;   // leading space in bytes

  if(j)
     {      
       for (i=0; i<j;i++)                       // for each column
         {
         for (yy=y; yy<y+BIG_FONT_HEIGHT; ++yy) // for each row of column
           {
            draw_pixel(x+i, yy, cl>>8);         // draw bg colour
           }
         }
      }

        draw_big_string(j+x,y,s,cl);            // text
        l = j+x+((strlen(s)*BIG_FONT_WIDTH));   // end of text                            

          for (; l<len+x; ++l)                      // for each column starting at 'l', draw bg color until end
           {
            for (yy=y; yy<y+BIG_FONT_HEIGHT; ++yy) // for each row of column
             {
              draw_pixel(l, yy, cl>>8);          // draw bg color
             }
           }
}
// -------------------------------------------------------------------
...
{
    if((*str == '!') || (*str=='#') || (*str==' ')|| (*str=='@')) str++;     // ignore colour format character

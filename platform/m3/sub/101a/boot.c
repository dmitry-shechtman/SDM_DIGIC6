 ...
 */		
		// args[0] = (unsigned)"LoadScript";
		// args[1] = (unsigned)"A/extend.m";
		// call_func_ptr(_ExecuteEventProcedure,args,2);			
		...
		// *(int*)0xd20b0810  = 0x4d0002;	 // Orange Led = on
		 ...
		 // args[0] = (unsigned)"UIFS_WriteFirmInfoToFile";
		 // args[0] = (unsigned)"UIFS_PostBatAuthNG";
		 // args[1] = (unsigned) 0 ;
		 // call_func_ptr(_ExecuteEventProcedure,args,1);
		 // *(int*)0xd20b0810  = 0x4d0002;	 // Orange Led = on
...
char voltagestr [10];
unsigned args[6];
char *program = 0x41741000; //0x41441000;
//FILE * f;
int fd;
...
while (1){
	_SleepTask(300);
		if (( *(int*)(0xD20BF4A0) & 0x00002000) == 0){ 
		...
		fd = _open( "A/MIT2.BIN" , O_RDONLY, 0777 );	
		*(int*)0xd20b0810  = 0x4d0002;	 // Orange Led = on
		//_Read(fd, program , (1472 * 480) );		
		_close(fd);
		*(int*)0xd20b0810  = 0x4c0003;	 // Orange Led = off
...
void CreateTask_spytask()
{
	...
	if ( *(int*)(0x9e5c) & 0x00600000 )  
		...
		else _CreateTask("SpyTask", 0x19, 0x2000, spytask, 0);
		}
}
